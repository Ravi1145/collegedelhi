...
export default function NIRFInsightsPage() {
  ...
  const nirfPageSchema = useMemo(() => ({
    "@context": "https://schema.org",
    "@type": "WebPage",
    name: "Delhi College Rankings 2026 — NIRF & NAAC Insights",
    description: "Explore NIRF rankings, NAAC grades, and placement data for all major Delhi colleges — all in one place.",
    ...
    mainEntity: {
      "@type": "ItemList",
      name: "NIRF Ranked colleges in Delhi",
      numberOfItems: allColleges.filter(c => c.nirfRank).length,
      itemListElement: allColleges
        .filter(c => c.nirfRank)
        ...
        .map((c, idx) => ({
          "@type": "ListItem",
          position: idx + 1,
          item: { "@type": "EducationalOrganization", name: c.name, url: `${BASE_URL}/colleges/${c.slug}` },
        })),
    },
  }), [allColleges])
  ...
  const topByPlacement = [...allColleges].sort((a, b) => b.avgPlacement - a.avgPlacement).slice(0, 5)
  const naacDistribution = allColleges.reduce<Record<string, number>>((acc, c) => {
  ...
  const SortIcon = ({ col }: { col: SortKey }) => {
    ...
    return sortAsc ? <ChevronUp className="w-3 h-3 text-red-600" /> : <ChevronDown className="w-3 h-3 text-red-600" />
  }
    ...
    <div className="bg-surface min-h-screen">
      ...
      <div className="bg-gradient-to-br from-[#0A1628] to-[#1E3A5F] py-12 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold text-white mb-3">
            Delhi College Rankings &amp; Accreditation
          </h1>
          <p className="text-blue-200 max-w-xl mx-auto">
            Explore NIRF rankings, NAAC grades, and placement data for all major Delhi colleges — all in one place.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label: "Colleges Listed", value: allColleges.length.toString(), color: "text-blue-700" },
            { label: "NIRF Ranked", value: allColleges.filter((c) => c.nirfRank).length.toString(), color: "text-purple-700" },
            { label: "NAAC A+ / A++", value: allColleges.filter((c) => c.naac === "A+" || c.naac === "A++").length.toString(), color: "text-green-700" },
            { label: "Avg Placement", value: allColleges.length ? formatINR(Math.round(allColleges.reduce((s, c) => s + c.avgPlacement, 0) / allColleges.length)) : "N/A", color: "text-red-800" },
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <h2 className="text-sm font-bold text-gray-900 mb-4 flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-red-600" /> Top 5 by Avg Placement
            </h2>
            <div className="space-y-3">
              {topByPlacement.map((c, i) => (
                <div key={c.slug} className="flex items-center gap-3">
                  <div className="w-7 h-7 rounded-full bg-red-50 flex items-center justify-center text-xs font-bold text-red-700 flex-shrink-0">
                    {i + 1}
                  </div>
                ...
                <div key={grade} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-gray-100 rounded-full h-2">
                    <div
                      ...
                      style={{ width: `${allColleges.length ? (count / allColleges.length) * 100 : 0}%` }}
                    />
                  </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="space-y-3 text-sm">
              {[
                { label: "Best NIRF Rank in Delhi", value: (() => { const top = allColleges.filter(c => c.nirfRank).sort((a, b) => (a.nirfRank ?? 0) - (b.nirfRank ?? 0))[0]; return top ? `#${top.nirfRank} — ${top.shortName}` : 'N/A' })() },
                { label: "Government Colleges", value: `${allColleges.filter(c => c.type === "Government").length} listed` },
                { label: "Deemed Universities", value: `${allColleges.filter(c => c.type === "Deemed").length} listed` },
                { label: "Autonomous Institutes", value: `${allColleges.filter(c => c.type === "Autonomous").length} listed` },
                { label: "Private Colleges", value: `${allColleges.filter(c => c.type === "Private").length} listed` },
              ].map(({ label, value }) => (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-5 flex flex-col sm:flex-row gap-3">
            ...
            {(["All", "Engineering", "MBA", "Medical"] as StreamFilter[]).map((s) => (
              <button
                ...
                className={cn(
                  ...
                  stream === s ? "bg-red-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                )}
              >
                {s}
              </button>
          ...
          <input
            ...
            className="border border-gray-200 rounded-xl px-3 py-1.5 text-sm outline-none focus:border-red-400 sm:w-56"
          />
        </div>
                ...
                {rankedColleges.length > 0 && (
                  <>
                    ...
                    {rankedColleges.map((college, idx) => (
                      <tr key={college.slug} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-2 sm:px-4 py-3 text-center">
                          <Link href={`/colleges/${college.slug}`} className="text-xs text-red-700 font-semibold hover:underline flex items-center justify-center gap-1">
        ...
        <div className="mt-6 bg-gradient-to-r from-[#0A1628] to-[#1E3A5F] rounded-2xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-3">
            ...
            <Link href="/counselling" className="bg-red-600 hover:bg-red-700 text-white font-bold px-4 py-2.5 rounded-xl text-sm whitespace-nowrap transition-colors">
              Free Counselling →
            </Link>
          </div>
        </div>
