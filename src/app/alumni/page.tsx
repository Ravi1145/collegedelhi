...
export const metadata: Metadata = genMeta({
  ...
  keywords: [
    ...
    "ask alumni Delhi college", "college alumni network Delhi",
    "Delhi engineering alumni", "mba alumni Delhi",
  ],
})
...
function Initials({ name, className = "" }: { name: string; className?: string }) {
  ...
  const colors   = ["bg-blue-100 text-blue-700", "bg-purple-100 text-purple-700", "bg-green-100 text-green-700", "bg-red-100 text-red-800"]
    ...
    <main className="bg-white min-h-screen">
      ...
      <section className="bg-surface py-10">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <h2 className="text-xl font-extrabold text-gray-900 mb-2">Have questions about a Delhi college?</h2>
          <p className="text-gray-500 text-sm mb-5">Browse Q&amp;A from real students or ask your own question.</p>
          <div className="flex flex-col sm:flex-row justify-center gap-3">
            <Link href="/qa" className="bg-red-600 hover:bg-red-700 text-white font-bold px-6 py-3 rounded-2xl text-sm transition-colors">
              Browse All Q&amp;A
            </Link>
            <Link href="/ask" className="bg-white border border-gray-200 hover:border-red-300 text-gray-700 font-semibold px-6 py-3 rounded-2xl text-sm transition-colors">
              Ask a Question
            </Link>
          </div>
        </div>
