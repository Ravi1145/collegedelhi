/**
 * lib/ai/prompts.ts - Structured AI prompt templates for CollegeDelhi
 *
...
export const COLLEGE_GENERATION_SYSTEM = `You are an expert Indian education data researcher with deep knowledge of Delhi colleges, NAAC grades, NIRF rankings, JEE/NEET/CAT/CUET admission processes, and placement statistics.
...
Return this exact JSON structure (no extra fields, all monetary values as integers in INR):
{
  ...
  "entrance_exams": ["JEE Main", "CUET", "relevant exams for Delhi"],
  ...
  "seo_keywords": ["10 focus keywords like 'best engineering college Delhi'"],
  "meta_title": "College Name - Fees, Placements, Courses 2026 | CollegeDelhi",
  ...
  "details": {
    ...
    "admission_process": [
      { "step": 1, "title": "Entrance Exam", "description": "Appear for JEE Main / CUET / relevant exam" },
      { "step": 2, "title": "Merit List", "description": "Institution publishes merit list based on exam score" },
      { "step": 3, "title": "Counselling", "description": "Participate in centralised counselling rounds" },
    ...
    "scholarships": [
      {
        ...
        "eligibility": "Delhi domicile, family income < ₹8L/yr",
...
export const BLOG_GENERATION_SYSTEM = `You are an expert education content writer specializing in Delhi college admissions, Indian engineering and MBA colleges, and student counseling. You write clear, factual, SEO-optimized articles for students and parents.
...
Return this exact JSON:
{
  ...
  "body": "Full HTML article with proper <h2>, <h3>, <p>, <ul>, <ol> tags. Include: intro paragraph, 4-6 H2 sections, a FAQ section with 5 Q&As, and a conclusion with CTA to contact CollegeDelhi counselors.",
...
export const CITY_PAGE_SYSTEM = `You are a Delhi education expert who creates informative, SEO-optimized city landing pages for college discovery. Content must be unique per city-stream combination, factually accurate, and helpful for students.

Return ONLY valid JSON. No markdown or extra text.`

export function buildCityPagePrompt(city: string, stream: string, topColleges: string[]): string {
  return `Generate content for a landing page: "${stream} Colleges in ${city}, Delhi"

Top colleges in this city for ${stream}: ${topColleges.join(', ')}

Return this exact JSON:
{
  "title": "${stream} Colleges in ${city} - Fees, Rankings & Admissions 2026",
  ...
  "meta_title": "Best ${stream} Colleges in ${city} 2026 - Fees, Cutoff & Admissions | CollegeDelhi",
  "meta_desc": "Explore top ${stream} colleges in ${city} with fees, placements, NAAC grades and admission details. Compare and get free counseling. Updated 2026."
}`
}
...
export function buildFAQPrompt(collegeName: string, stream: string): string {
  return `Generate 10 comprehensive FAQs for: "${collegeName}" (${stream} college in Delhi)
