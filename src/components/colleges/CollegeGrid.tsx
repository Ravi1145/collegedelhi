...
export default function CollegeGrid() {
  ...
  // â”€â”€ Fetch ONLY from backend API — no static data â”€â”€
  useEffect(() => {
    let cancelled = false
    async function load() {
      setLoading(true)
      try {
        ...
        const json = res.ok ? await res.json() : { colleges: [] }
        if (!cancelled) {
          const colleges: College[] = json.colleges ?? []
          const featured = colleges.filter((c) => c.isFeatured)
          const others   = colleges.filter((c) => !c.isFeatured)
          setAllColleges([...featured, ...others])
        }
      } catch {
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-center h-64 gap-3 text-gray-400">
              ...
              <span className="text-sm">Loading colleges from databaseâ€¦</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {searchQuery && (
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900">
            Search results for &quot;<span className="text-red-700">{searchQuery}</span>&quot;
          </h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1 border-l border-gray-200 pl-3">
                <button
                  ...
                  className={cn("p-1.5 rounded", viewMode === "grid" ? "bg-red-100 text-red-700" : "text-gray-400 hover:text-gray-600")}
                >
                  <Grid3X3 className="w-4 h-4" />
                </button>
                <button
                  onClick={() => setViewMode("list")}
                  className={cn("p-1.5 rounded", viewMode === "list" ? "bg-red-100 text-red-700" : "text-gray-400 hover:text-gray-600")}
                >
          ...
          {filteredColleges.length === 0 ? (
            ...
            <div className="space-y-5">
              ...
              {filteredColleges.length > 5 && (
                <InlineLeadForm context="Browse colleges in Delhi" />
              )}
              {filteredColleges.length > 5 && (
                <div className={cn(
                  "grid gap-5",
