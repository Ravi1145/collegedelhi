      ...
      {options.map((option) => (
        <label key={option} className="flex items-center gap-2 cursor-pointer group">
          ...
          <span className={cn(
            ...
            selected.includes(option) ? "text-red-700 font-medium" : "text-gray-600 group-hover:text-gray-900"
          )}>
            {option}
          </span>
        </label>
...
export default function CollegeFilters({ onFiltersChange, className }: CollegeFiltersProps) {
  ...
  const filterContent = (
    <div className={cn("bg-white rounded-2xl border border-gray-100 p-5", className)}>
      <div className="flex items-center justify-between mb-5">
        ...
        {activeCount > 0 && (
          <button
            ...
            className="flex items-center gap-1 text-xs text-gray-500 hover:text-red-700 transition-colors"
          >
      ...
      <FilterSection title="Location in DELHI" defaultOpen={false}>
        <CheckboxGroup
          options={DELHI_AREAS}
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="px-5 pb-8 pt-2 flex gap-3">
              ...
              <button onClick={() => setMobileOpen(false)} className="flex-1 bg-red-600 text-white rounded-xl py-3 text-sm font-semibold">
