  ...
  return (
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map(n => (
        ...
        >
          <Star
            className={`${sz} transition-colors ${n <= (hover || value) ? "fill-red-400 stroke-red-400" : "stroke-gray-300 fill-transparent"}`}
          />
        </button>
      ))}
    </div>
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-red-400 to-red-700 flex items-center justify-center text-white font-bold text-sm shrink-0">
      ...
      {isLong && (
        <button
          ...
          className="text-xs text-red-700 font-medium flex items-center gap-1 mb-3"
        >
...
function SubmitForm({ collegeSlug, collegeName, onSuccess }: { collegeSlug: string; collegeName: string; onSuccess: () => void }) {
  ...
  const [form, setForm] = useState({ student_name: "", userEmail: "", course: "", year: "", title: "", review_body: "", pros: "", cons: "" })
