  ...
  if (variant === "compact") {
    return (
      <Link href={`/colleges/${college.slug}`} className="block">
        <div className="flex items-center gap-3 p-3 bg-white rounded-xl border border-gray-100 hover:border-red-200 hover:shadow-md transition-all group">
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-red-600 to-red-700 flex-shrink-0 flex items-center justify-center overflow-hidden">
            {college.image ? (
              <Image
                src={college.image}
        ...
        >
          ...
          {college.isFeatured && (
            <div className="absolute top-2 left-2 z-20">
              <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-bold rounded-full bg-red-600 text-white shadow-md">
          ...
          <div className="relative z-10 flex items-start gap-3 pr-10">
            ...
            <div className="shrink-0 w-14 h-14 rounded-xl bg-white shadow-md flex items-center justify-center overflow-hidden">
              {college.logo_base64 ? (
                <img src={college.logo_base64} alt={`${college.name} logo`} className="w-full h-full object-contain p-1" />
              ) : college.image ? (
                <Image
                  src={college.image}
                  alt={`${college.name} logo`}
            ...
            <div className="flex-1 min-w-0 pt-0.5">
              <h3
                className="text-white font-bold text-sm leading-snug line-clamp-2 group-hover:text-red-300 transition-colors"
          ...
          <div className="relative z-10 flex flex-wrap gap-1.5 mt-3">
            ...
            {college.nirfRank ? (
              <span className="text-xs px-2.5 py-0.5 rounded-full font-bold bg-red-600 text-white">
