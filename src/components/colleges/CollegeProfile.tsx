  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="flex items-center gap-2 px-6 py-4 border-b border-gray-50">
        {Icon && <Icon className="w-4 h-4 text-red-600 flex-shrink-0" />}
...
function TableRow({ label, value, highlight }: { label: string; value: React.ReactNode; highlight?: boolean }) {
  return (
    <tr className={cn("border-b border-gray-50 last:border-0", highlight && "bg-red-50/50")}>
...
export default function CollegeProfile({ college, details }: CollegeProfileProps) {
  ...
  // Fetch backend courses linked to this college
  useEffect(() => {
    if (!college.slug) return
    fetch(`https://college-delhi-backend.vercel.app/api/courses/college/${college.slug}`)
      .then(r => r.json())
    ...
    <div>
      ...
      <div className="bg-gradient-to-br from-[#0A1628] to-[#1E3A5F] text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-0">
          ...
          <div className="flex items-start gap-5 mb-5">
            {/* Logo */}
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-white flex-shrink-0 flex items-center justify-center shadow-lg overflow-hidden border border-white/20">
              {college.logo_base64 ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={college.logo_base64}
                  alt={`${college.shortName} logo`}
                  className="w-full h-full object-contain p-1"
                />
              ) : (
                <span className="text-gray-700 font-extrabold text-xs sm:text-sm leading-tight text-center px-1">
                  {college.shortName}
                </span>
              )}
            </div>
            ...
            <div className="hidden sm:flex flex-col gap-2 flex-shrink-0">
              <button
                ...
                className="bg-red-600 hover:bg-red-700 text-white font-semibold text-sm px-5 py-2.5 rounded-xl transition-colors"
              >
                Enquire Now
              </button>
              {college.website && (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
            {[
              ...
              { label: "Rating", value: `${college.rating}/5 â­`, icon: Star },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <>
                  ...
                  {college.highlights.length > 0 && (
                    <div className="bg-red-50 rounded-xl p-4">
                      ...
                      <ul className="space-y-2">
                        {college.highlights.map((h: any, i) => (
                ...
                <SectionCard title={`${college.shortName} Highlights`} icon={Layers}>
                  <table className="w-full">
                    <tbody>
                      ...
                      {college.website && <TableRow label="Official Website" value={<a href={college.website} target="_blank" rel="noopener noreferrer" className="text-red-600 hover:underline text-xs">{college.website}</a>} />}
                    </tbody>
                  </table>
                </SectionCard>
                ...
                <section className="geo-content bg-blue-50 rounded-2xl border border-blue-100 p-6 text-sm text-gray-700 leading-relaxed space-y-3">
                  ...
                  <p className="text-xs text-gray-400 pt-1">
                    Source: collegedelhi.com · Data verified April 2026
                  </p>
                </section>
              </>
            )}

            {/* â”€â”€ COURSES & FEES TAB â”€â”€ */}
            {activeTab === "courses" && (
              <>
                {/* Backend courses — linked via topColleges */}
                {backendCourses.length > 0 && (
                  <SectionCard title="Courses Available at This College" icon={GraduationCap}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {backendCourses.map((c: any) => (
                        <Link key={c._id} href={`/courses/${c.slug}`}
                          className="group border border-gray-100 rounded-xl p-4 hover:border-red-300 hover:bg-red-50/30 transition-all">
                          <div className="flex items-start justify-between mb-2">
                            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-red-100 text-red-800">{c.level}</span>
                            <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-red-600 transition-colors" />
                          </div>
                          <p className="font-bold text-gray-900 text-sm group-hover:text-red-700 transition-colors">{c.name}</p>
                          {c.fullName && <p className="text-xs text-gray-500 mt-0.5 line-clamp-1">{c.fullName}</p>}
                          <div className="flex items-center gap-4 mt-3 text-xs text-gray-400">
                            {c.duration && <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{c.duration}</span>}
                            {c.fees?.min > 0 && <span className="flex items-center gap-1"><IndianRupee className="w-3 h-3" />{c.fees.min >= 100000 ? `${(c.fees.min/100000).toFixed(1)}L` : `${(c.fees.min/1000).toFixed(0)}K`}+</span>}
                            {c.avgSalary?.min > 0 && <span className="flex items-center gap-1 text-green-600"><TrendingUp className="w-3 h-3" />{(c.avgSalary.min/100000).toFixed(1)} LPA+</span>}
                          </div>
                          {c.entranceExams?.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-2">
                              {c.entranceExams.slice(0, 3).map((e: string, i: number) => (
                                <span key={i} className="bg-gray-100 text-gray-600 text-[10px] px-1.5 py-0.5 rounded">{e}</span>
                              ))}
                            </div>
                          )}
                        </Link>
                    ...
                    <div className="mt-4 pt-4 border-t">
                      <Link href="/courses" className="text-sm text-red-700 hover:text-red-800 font-medium flex items-center gap-1">
                ...
                {/* Structured fees table from admin */}
                  ...
                  <SectionCard title="Courses Offered" icon={BookOpen}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {college.courses.map(c => (
                        <div key={c} className="border border-gray-100 rounded-xl p-4 hover:border-red-200 transition-colors">
                          <BookOpen className="w-4 h-4 text-red-600 mb-2" />
                          ...
                          <tbody>
                            {details.placements.stats.map((s, i) => (
                              <tr key={i} className="border-t border-gray-100">
                                ...
                                <td className="py-3 px-3 text-right font-bold text-red-700">{formatCurrency(s.highest_package)}</td>
                  ...
                  <SectionCard title="Placement Statistics" icon={TrendingUp}>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-5">
                      {[
                        ...
                        { label: "Highest Package", value: formatCurrency(college.highestPlacement), color: "text-red-700" },
                          ...
                          {details.rankings.map((r, i) => (
                            <tr key={i} className="border-t border-gray-100">
                              ...
                              <td className="py-3 px-3 text-center">
                                <span className="bg-red-100 text-red-800 font-bold text-xs px-2 py-0.5 rounded-full">
                  ...
                  <SectionCard title="Rankings" icon={Trophy}>
                    {college.nirfRank && (
                      <div className="bg-red-50 rounded-xl p-5 text-center">
                        <p className="text-4xl font-extrabold text-red-700 mb-1">#{college.nirfRank}</p>
                      ...
                      {details.alumni.map((a, i) => (
                        <div key={i} className="flex items-center gap-3 border border-gray-100 rounded-xl p-4">
                          <div className="w-10 h-10 bg-red-100 rounded-full flex items-center justify-center flex-shrink-0">
                            <span className="text-red-700 font-bold text-sm">{a.name.charAt(0)}</span>
                          </div>
                          <div>
          ...
          {/* â”€â”€ Sticky Sidebar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
                ...
                <div className="bg-gradient-to-br from-red-600 to-red-700 rounded-2xl p-4 text-white">
                  ...
                  <p className="text-red-100 text-xs mb-3">
                    {college.stream === "Engineering"
                      ? `JEE Main / JEE guidance for ${college.shortName ?? college.name}. 100% free.`
                  ...
                  <Link href="/counselling" className="block text-center bg-white text-red-700 font-bold text-sm rounded-xl py-2.5 hover:bg-red-50 transition-colors">
                    Book Free Session
                  </Link>
                </div>
              </div>
      ...
      <section className="border-t bg-white py-10 px-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              { label: `Best ${college.stream} colleges in Delhi`, href: `/colleges-in-delhi/${college.stream.toLowerCase().replace(/[^a-z]/g, '-')}-colleges-DELHI` },
              { label: `${college.type} colleges in Delhi`, href: `/colleges-in-delhi/${college.type.toLowerCase()}-colleges-DELHI` },
              { label: "Colleges with Best Placements DELHI", href: "/colleges-in-delhi/top-placement-colleges-DELHI" },
              ...
              ...(college.entranceExams[0] ? [{ label: `${college.entranceExams[0]} Colleges DELHI`, href: `/colleges-in-delhi/${college.entranceExams[0].toLowerCase().replace(/[^a-z]/g, '-')}-colleges-DELHI` }] : []),
