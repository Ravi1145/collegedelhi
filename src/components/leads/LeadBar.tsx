      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2 flex items-center gap-3 flex-wrap sm:flex-nowrap">
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} className="flex items-center gap-2 flex-1 justify-end sm:justify-end flex-wrap">
            <label className="flex items-center gap-1 text-[10px] text-white/70 cursor-pointer shrink-0">
              <input
                type="checkbox"
              ...
              <span>
                Agree{" "}
                <a href="/terms" className="underline hover:text-white">T&amp;C</a>
              </span>
            </label>
            <input
              type="tel"
              ...
              className="w-40 sm:w-48 px-3 py-1 rounded-lg text-gray-900 text-xs placeholder-gray-500 outline-none focus:ring-2 focus:ring-red-400 bg-white"
              maxLength={10}
            />
            <button
              type="submit"
              disabled={loading || phone.length !== 10 || !agreedToTerms}
              className="px-3 py-1 bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white text-xs font-bold rounded-lg transition-colors shrink-0"
            >
              {loading ? "…" : "Get Guide"}
            </button>
          </form>
